...
int wxSerialPort::InQueueSize(void)
{
	int count = 0;
	ioctl(fd,CTB_SER_GETINQUE,&count);
	return count;
}
int wxSerialPort::OutQueueSize(void)
{
	return 0;
}
void wxSerialPort::FlushIn(void)
{
	tcflush(fd,TCIFLUSH);
}
void wxSerialPort::FlushOut(void)
{
	tcflush(fd,TCOFLUSH);
}
